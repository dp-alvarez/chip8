...
def render():
	...
	newscreen = newscreen.repeat(gui.config.window.draw_size, axis=0).repeat(gui.config.window.draw_size, axis=1)
...
def init():
	...
	pyg_screen = pyg.surface.Surface(gui.config.window.screen_size)
	pyg_screen_array = pyg.surfarray.array2d(pyg_screen)
	mask = np.zeros(pyg_screen_array.shape, dtype='bool')
	draw_color_mapped = pyg_screen.map_rgb(gui.config.window.draw_color)
	window_bg_mapped = pyg_screen.map_rgb(gui.config.window.window_bg)
